...
const AuntFlo = () => {
  ...
  const fetchPeriodRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://121.4.22.55:5201/api/auntflo/records?username=${encodeURIComponent(username)}&startDate=${formatDate(startDate)}&endDate=${formatDate(endDate)}`
      );
  ...
  const saveRecord = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('http://121.4.22.55:5201/api/auntflo/records', {
        method: 'POST',
        headers: {
