...
const Home = () => {
    // 你的所有 state 和 hooks 保持不变
    const { dispatch, currentSong } = useMusic(); // 假设你之后会用到 currentSong
        ...
        const fetchMusics = async () => {
            ...
            try {
                ...
                const newMusics = response.data.data.map(song => ({
                    ...
                    artist: song.artist,
                    genre: song.genre,
                    liked: song.liked || false, // 确保有 liked 属性
    ...
    return (
        <div className={styles.home}>
            <div className={styles.allMusicSection}>
                {/* 【修改】: 简化了 sectionHeader 的内部结构 */}
                <div className={styles.sectionHeader}>
                    {/* 1. 标题 */}
                    <h2 className={styles.sectionTitle}>音乐库 ({musics.length})</h2>
                ...
                {viewMode === 'table' ? (
                    <MusicTableView
                        ...
                        currentSong={currentSong} // 传递 currentSong
                    />
                ) : (
                    <MusicGridView
                        musics={musics}
