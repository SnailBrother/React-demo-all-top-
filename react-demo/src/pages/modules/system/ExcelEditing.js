        ...
        <div className="excel-grid">
          <div className="excel-row header-row">
            ...
            {headers.map((_, colIndex) => (
              <div key={colIndex} className="excel-cell header-cell">{getColumnLetter(colIndex + 1)}</div>
            ))}
          </div>
              ...
              {row.map((cell, colIndex) => {
                ...
                if (cell.dataValidation?.type === 'list' && cell.dataValidation.options?.length > 0) {
                  if (editingCell === address) {
                    return (
                      <div key={cell.id} className={`excel-cell ${isActive ? 'active' : ''} ${highlight ? 'highlighted' : ''}`} onBlur={() => setEditingCell(null)}>
                        <select
                          autoFocus
                          value={cell.value || ''}
    ...
    <div className="excel-editor">
      ...
      <div className="toolbar">
        <div className="toolbar-section">
          ...
          <button className="toolbar-btn info" onClick={redo} disabled={(historyIndex[currentSheetName] || -1) >= (history[currentSheetName]?.length - 1 || 0)}>
            <Icon type="save" /> 恢复
          </button>
        </div>
      </div>
        ...
        <div className="sheet-tabs">
          <div className="sheet-tabs-container">
            {sheetNames.map((name, index) => (
              <button
                key={index}
                className={`sheet-tab ${currentSheetName === name ? 'active' : ''}`}
                onClick={() => setCurrentSheetName(name)}
                disabled={loading}
              >
                {name}
                {currentSheetName === name && <span className="tab-indicator"></span>}
              </button>
            ))}
          </div>
        </div>
