...
const ThemeSettings = () => {
    ...
    useEffect(() => {
        const loadThemeSettings = async () => {
            try {
                const response = await axios.get('http://121.4.22.55:5201/api/getthemesettings', {
                    params: { username },
                });
    ...
    const saveThemeSettings = async () => {
        try {
            ...
            // 保存主题设置
            const themeResponse = await axios.post('http://121.4.22.55:5201/api/savethemesettings', formData, {
                headers: {
            ...
            if (themeResponse.data.success) {
                ...
                if (backgroundImage) {
                    ...
                    const imageResponse = await axios.post(`http://121.4.22.55:5201/backend/api/uploadchatbackground?username=${username}`, imageFormData, {
                        headers: {
                            'Content-Type': 'multipart/form-data',
                        },
                    });
