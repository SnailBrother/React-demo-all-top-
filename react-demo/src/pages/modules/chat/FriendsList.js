            ...
            try {
                ...
                const friendsWithHeadImagesAndUnreadCounts = await Promise.all(
                    uniqueFriends.map(async (name, index) => {
                        try {
                            const headImageResponse = await axios.get('http://121.4.22.55:5201/api/getuserheadimage', {
                            ...
                            // 获取未读消息数
                            const unreadResponse = await axios.get('http://121.4.22.55:5201/api/messages');
