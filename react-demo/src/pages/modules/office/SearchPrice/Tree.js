...
function Tree() {
    ...
    const fetchRandomTrees = async () => {
        try {
            setIsLoading(true);
            const response = await axios.get('http://121.4.22.55:5201/api/getRandomTrees');
    ...
    const handleSearchSubmit = async () => {
        ...
        try {
            const response = await axios.get(
                `http://121.4.22.55:5201/api/searchTrees?term=${encodeURIComponent(searchTerm)}&page=${currentPage}&pageSize=${itemsPerPage}`
            );
        ...
        if (page >= 1 && page <= totalPages) {
            ...
            try {
                ...
                if (isSearching) {
                    // 如果是搜索状态，保持搜索
                    const response = await axios.get(
                        `http://121.4.22.55:5201/api/searchTrees?term=${encodeURIComponent(searchTerm)}&page=${page}&pageSize=${itemsPerPage}`
                    );
                    setFilteredTrees(response.data.results);
                } else {
                    // 如果不是搜索状态，获取新的分页数据
                    const response = await axios.get(
                        `http://121.4.22.55:5201/api/getTrees?page=${page}&pageSize=${itemsPerPage}`
                    );
                    setFilteredTrees(response.data.results);
                }
            } catch (error) {
    ...
    //         setIsLoading(true);
    //         const response = await axios.get(
    //             `http://121.4.22.55:5201/api/getTrees?page=${page}&pageSize=${size}`
    //         );
    //         setFilteredTrees(response.data.results);
    //         setTotalCount(response.data.totalCount);
    ...
    const handleItemsPerPageChange = async (e) => {
        ...
        try {
            ...
            if (isSearching) {
                // 如果是搜索状态，保持搜索
                const response = await axios.get(
                    `http://121.4.22.55:5201/api/searchTrees?term=${encodeURIComponent(searchTerm)}&page=1&pageSize=${newSize}`
                );
                setFilteredTrees(response.data.results);
                setTotalCount(response.data.totalCount);
                ...
                // 如果不是搜索状态，获取新的分页数据
                const response = await axios.get(
                    `http://121.4.22.55:5201/api/getTrees?page=1&pageSize=${newSize}`
                );
                setFilteredTrees(response.data.results);
                setTotalCount(response.data.totalCount);
                setTotalPages(Math.ceil(response.data.totalCount / newSize));
    ...
    const handleDeleteClick = async (id) => {
        try {
            await axios.delete(`http://121.4.22.55:5201/api/deleteTree/${id}`);
            // Socket通知会自动更新数据
            alert('删除成功');
        } catch (error) {
    ...
    const handleFormSubmit = async (event) => {
        ...
        try {
            if (editTree) {
                await axios.put(`http://121.4.22.55:5201/api/updateTree/${editTree.id}`, updatedTree);
            } else {
                await axios.post('http://121.4.22.55:5201/api/addTree', updatedTree);
                alert('新增成功');
            }
        } catch (error) {
    ...
    const handleExcelUpload = async () => {
        ...
        try {
            const response = await axios.post('http://121.4.22.55:5201/api/uploadTreesExcel', { data: excelData });
