...
function Equipment() {
  ...
  const fetchEquipmentData = async () => {
    try {
      // 发送 GET 请求获取设备数据
      const response = await axios.get("http://121.4.22.55:5201/api/getMachineryEquipmentPricesTable");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedEquipment) {
        // 更新设备
        await axios.put(`http://121.4.22.55:5201/api/updateMachineryEquipmentPricesTable/${selectedEquipment.id}`, newEquipment);
      } else {
        // 添加新设备
        await axios.post("http://121.4.22.55:5201/api/addMachineryEquipmentPricesTable", newEquipment);
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://121.4.22.55:5201/api/deleteMachineryEquipmentPricesTable/${id}`);
      fetchEquipmentData(); // 刷新设备列表
    } catch (error) {
